...
- Ready = 状态是否为「待测试 / 测试中 / 待验收」及之后
- Ready Date = 进入「待测试」的日期
- 默认提测截止日期 = Sprint **第二周第一个工作日**（开始日 + 7 天后的首个工作日）。开发排期仅存档，不参与判定
- 日报 Story「vs 计划」：已设计划线时，按各启用环节的**窗口结束日门槛**判定（结束当天仍未达标 → Behind；开发中/联调中若未到「开发提测」门槛也会标 Behind）。未设计划线时，退回默认提测截止日
- 已提测：**晚于**截止日期才 Comment=`提测Delay`（当天提测不算）。未设计划线时截止日期为第二周首个工作日；已设「开发提测」环节时详情「计划提测」用该环节结束日
